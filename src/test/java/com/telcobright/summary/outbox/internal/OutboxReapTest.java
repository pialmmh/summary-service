...
class OutboxReapTest {
    ...
    private static final List<String> BEANS = List.of("dailyCdrSummary", "hourlyCdrSummary");
    ...
    void deletes_up_to_the_min_offset_across_beans() {
        ...
        outbox.seed(2, "x");
        outbox.seed(3, "x");
        outbox.advanceOffset("cdr", "dailyCdrSummary", 3);
        outbox.advanceOffset("cdr", "hourlyCdrSummary", 2);   // hourly lags
    ...
    void keeps_everything_until_every_bean_has_progressed() {
        FakeOutboxStore outbox = new FakeOutboxStore();
        outbox.seed(1, "x");
        outbox.advanceOffset("cdr", "dailyCdrSummary", 1);    // hourly has NO offset row yet
